...
Then $(m+1)/(n-m) \leq 1/\eps$ and $\abs{s_n-s_i} < M\eps$. For sufficiently large $n$, 

\[
\abs{s_n-\sigma_n} \leq \eps + \frac{n-m}{n-m} \frac{(n-m-1)M}{m+2} \leq \eps + M\eps
\]
...
(d) Let the rapidity of convergence of this process as $\eps'_n$ in Exercise 16. Let $r' = \eps'/\beta < 1$. Take $r$ for $r\geq r'$, $(r')^{2n-n} << 1$. Then the previous method is way more convergent fast compared to current one.
