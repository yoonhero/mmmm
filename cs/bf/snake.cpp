...
bool illegalMove(Move move, Move prevMove) { return abs(move - prevMove) == 2; }
...
void renderSnake(Renderer &renderer, Snake &snake) {
    for (const int snake_body : snake) {
        // renderer->renderRect(snake_body, GREEN);
        renderer.renderLattice(snake_body, GREEN);
    }
}
...
int main(void) {
    ...
    Renderer renderer(X_BLOCKS, Y_BLOCKS, BLOCK_SIZE);
    ...
    while (running) {
        ...
        renderer.renderLattice(applePtr, RED);
